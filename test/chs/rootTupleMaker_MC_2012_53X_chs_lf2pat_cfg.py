...
usePFIso( process )
switchJetCollection(process, jetCollection=cms.InputTag('pfJetsPFlow'),doJTA=True,doBTagging=True, jetCorrLabel=('AK5PFchs', inputJetCorrLabel),doType1MET=False, doJetID=True,
...
process.analysisPatJets = process.cleanPatJets.clone()
process.analysisPatJets.finalCut = cms.string("abs(eta)<2.5 & pt > 10")
process.patDefaultSequence.replace ( process.cleanPatJets, process.cleanPatJets + process.analysisPatJets )
...
runMEtUncertainties(
    process,
    jetCollection           = cms.InputTag('analysisPatJets'), 
...
# - process.smearedPatJets                     : pat jets     , energy resolution smeared to match data  <-- Recommended for analysis
# - process.smearedPatJetsresUp                : pat jets     , energy resolution smeared worse data
# - process.smearedPatJetsresDown              : pat jets     , energy resolution sharpened better than data
# - process.shiftedPatJetsenUpForCorrMEt       : pat jets     , energy scale shifted up  
# - process.shiftedPatJetsenDownForCorrMEt     : pat jets     , energy scale shifted down 
# 
# Notes:
...
#process.rootTuplePFJets.InputTag = cms.InputTag('smearedAnalysisPatJets')       
#process.rootTuplePFJets.InputTag = cms.InputTag('analysisPatJets')
process.rootTuplePFJets.InputTag = cms.InputTag('cleanPatJets')
#process.rootTuplePFJets.InputTagSmearedUp   = cms.InputTag('smearedAnalysisPatJetsResUp')                                 
#process.rootTuplePFJets.InputTagSmearedDown = cms.InputTag('smearedAnalysisPatJetsResDown')                                 
#process.rootTuplePFJets.InputTagScaledUp    = cms.InputTag('shiftedAnalysisPatJetsEnUpForCorrMEt')                                 
#process.rootTuplePFJets.InputTagScaledDown  = cms.InputTag('shiftedAnalysisPatJetsEnDownForCorrMEt')     
process.rootTuplePFJets.InputTagSmearedUp = cms.InputTag('cleanPatJets')
process.rootTuplePFJets.InputTagSmearedDown = cms.InputTag('cleanPatJets')
process.rootTuplePFJets.InputTagScaledUp    = cms.InputTag('cleanPatJets')
process.rootTuplePFJets.InputTagScaledDown  =  cms.InputTag('cleanPatJets')
#----------------------------------------------------------------------------------------------------
